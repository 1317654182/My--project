...
// 访客信息
const getlistFk = (async() => {
    increment();
    const url = "/appForm/GetViewData?viewName=visitorrecord_view_phone&getType=all&pageNum=1&pageSize=9999&order=descending&orderProp=appdate&searchData=+and+visitingtime+%3E%3D+%27"+dayjs(new Date()).format('YYYY-MM-DD')+"%27";
    await axios.get(url, {
        headers: {
        ...
        for (let i = 0; i < res.data.data.records.length; i++) {
            ...
            NewArr.value[i].index = NewArr.value[i].visitingtimes - NewArr.value[i].appdates
